...
Try {
 ...
 Catch [Microsoft.ActiveDirectory.Management.ADIdentityNotFoundException] { $ErrorCheck = $true;
 ...
 Catch [System.Management.Automation.RuntimeException] {$ErrorCheck = $true;  "У пользователя $CatchLogin некорректное значение CSAT: $CatchCsat в выгрузке \\dlc-fs-02\CSAT\Login CSAT.csv  Значение CSAT должно быть в промежутке от 0.1 до 5.0" | Out-File -FilePath "\\dlc-fs-02\CSAT\Log\SET_DispName\CSAT_Error_$(get-date -f yyyy-MM-dd).txt" -Append}
})

<#
ОПИСАНИЕ:
Скрипт загружает данные из файла \\*******\CSAT\Login CSAT.csv;
...
События связанные с ошибками во время работы скрипта регистрируются в файле журнала. \\*******\CSAT\Log\SET_DispName
...
$CSATinput = "********CSAT\Login CSAT.csv" # Файл Выгрузки CSAT



function SendAlert # Функция отправки alert ответственному лицу в случае возникновения ошибок
	{
	Send-MailMessage -FROM 'alert_CSAT_script@rolf.ru' -to $EMAI -Subject "Возникла ошибка при выполнении скрипта CSAT.ps1 на хосте rolf-hpsim." -body "Возникла ошибка при выполнении скрипта CSAT.ps1 на хосте rolf-hpsim. Файл журнала ********\CSAT\Log\SET_DispName\" –SmtpServer 'smtprelay.rolf.ru' -Encoding 'UTF8' 
	}

#Получение Данных из файла выгрузки CSAT
Try {$CSATData = Import-CSV -Path $CSATinput -delimiter ";" -Encoding Default}
catch [System.IO.FileNotFoundException]{$ErrorCheck = $true; "Отсутствует файл выгрузки CSAT ********\CSAT\Login CSAT.csv" | Out-File -FilePath "******\CSAT\Log\SET_DispName\CSAT_Error_$(get-date -f yyyy-MM-dd).txt" -Append}
...
$CSATData.ForEach({
		...
		If ($UserData.extensionattribute11 -eq $_.CSAT){} # Проверка на совпадение значения csat c 11м атрибутом. В случае совпадения изменение DisplayName не требуется. Отключить проверку при первом запуске скрипта.
			...
			$DisplayCSAT = $UserData.CN+ " (CSAT: "+$_.CSAT+" ⭐)" # Формирование DisplayName c CSAT
				if ($CSAT -lt 0.1 -or $CSAT -gt 5 -or $null ) # проверка на соответствие значения CSAT
					{
					$ErrorCheck = $true; "У пользователя $($_.Login) некорректное значение CSAT: $($_.CSAT) в выгрузке \\*******\CSAT\Login CSAT.csv  Значение CSAT должно быть в промежутке от 0.1 до 5.0" | Out-File -FilePath "\\*******\CSAT\Log\SET_DispName\CSAT_Error_$(get-date -f yyyy-MM-dd).txt" -Append
					}
				else 
					{ 
	...
	Catch [Microsoft.ActiveDirectory.Management.ADIdentityNotFoundException] 
		{ 
		$ErrorCheck = $true;
		"Ошибка получения данных пользователя $($CatchLogin) из AD: $($error[0])" | Out-File -FilePath "*************\CSAT\Log\SET_DispName\CSAT_Error_$(get-date -f yyyy-MM-dd).txt" -Append
		}
	Catch [System.Management.Automation.RuntimeException]
		{
		$ErrorCheck = $true;
		"У пользователя $CatchLogin некорректное значение CSAT: $CatchCsat в выгрузке \\*******\CSAT\Login CSAT.csv  Значение CSAT должно быть в промежутке от 0.1 до 5.0" | Out-File -FilePath "\\*******\CSAT\Log\SET_DispName\CSAT_Error_$(get-date -f yyyy-MM-dd).txt" -Append
		}
})

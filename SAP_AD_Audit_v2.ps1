...
#Send-MailMessage -Credential $creds –SmtpServer smtprelay.rolf.ru -FROM 'msk-sysadmins@rolf.ru' -To "smpolykov@rolf.ru" -Subject "Отчет по проблемным учетным записям пользователей в AD." -Body "Коллеги, доброе утро!`n`nВо вложении отчет по проблемным учетным записям пользователей в AD и инструкция по обработке отчёта.`nПросьба обработать файл SAP-AD.xlsx согласно инструкции.`nХорошего дня! " -Encoding 'UTF8' -Attachments  $SapADPath, $ManualPath
}
...
function sap_fio ([string]$surname,[string]$name,[string]$patronymic){

$SapFIO =$surname, $name,$patronymic -join " " 
while ($Sapfio[-1] -eq " "){$Sapfio=$Sapfio.Remove($Sapfio.Length - 1)}
return $Sapfio}
...
{
if ($displayName -match "CSAT"){$displayName=($displayname -replace " \(CSAT.*")}
return $displayName
}
...
#$AdDisableData=(Import-Csv $AdDataPath -Delimiter ";" -Encoding Default|where-object "Команда поддержки" -eq  "Disabled users")
$SapOnData=($SAPdata|where-object Sap_Status -eq  "Enabled")
#$SapPregnancy=(Import-Csv $SapDataPath -Delimiter ";" -Encoding Default |where-object Sap_Status -eq  "Pregnancy")
